...
export class Processor<T> {
  ...
  protected commandsOptions: CommandsOpts;

  constructor(
    commandsOptions: CommandsOpts,
    inputQueue: Queue<T>,
    outputQueue: Queue<T>
  ) {
...
export class Writer<T> {
  ...
  constructor(commandsOptions: CommandsOpts, outputQueue: Queue<T>) {
    this.commandsOptions = commandsOptions;
    this.outputQueue = outputQueue;
  }
...
export class JobComplete<T> {
  ...
  protected hooksOpts: HooksOpts;

  constructor(hooksOpts: HooksOpts, jobCompleteQueue: Queue<T>) {
